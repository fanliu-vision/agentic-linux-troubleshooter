...
R14-6 为阶段总结，新增 `docs/r14_structural_hardening_report.md`，汇总 R14-1 到 R14-5 的结构硬化结果、测试结果、安全边界、剩余风险和后续路线。
...
| -- | -- | -- |
| R14-1 | 完成 | 结构硬化规划完成 |
| R14-2 | 完成 | `project_status / runtime_health` 增强完成 |
| R14-3 | 完成 | retention / log rotation 设计与只读 inventory 完成 |
| R14-4 | 完成 | report / alert rate limit 与 flood control 完成 |
| R14-5 | 完成 | `seen_fingerprints` compact dry-run 完成 |
| R14-6 | 完成 | 阶段验收总结完成 |

R14 完成后建议进入 R15：自动恢复策略安全分层与 guarded `auto_recover` 设计。R15 应先做策略设计，不直接扩大恢复动作。
...
- R15：自动恢复策略安全分层与 guarded `auto_recover` 设计，在不越权的前提下细化 `diagnose_only`、`manual_escalation`、`safe_auto_recover`、`guarded_auto_recover`、`disabled` 等策略层级；
- R16：新增/细化故障域，在已有结构硬化和回归保护基础上扩展 detector 覆盖面；
- R17：CI、Runbook、交付质量，完善持续验证、操作手册、发布检查和交付验收材料。

建议进入 R15：自动恢复策略安全分层与 guarded `auto_recover` 设计。
