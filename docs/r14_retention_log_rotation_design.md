...
- `seen_fingerprints` compact 放到 R14-5，且 R14-5 只做 dry-run plan，不写回真实 state；
...
R14-5 的最小安全版本应只读取指定 `project_status.json` 与可选 `events.jsonl`，生成 keep/drop 计划、风险说明和审计报告。真实 compact 必须留到后续阶段，并满足先备份、再执行、再审计、可回滚。
...
- R14-5：做 `seen_fingerprints` compact dry-run，处理 persistent seen 长期增长的计划生成、keep/drop 统计和审计报告；
- retention 执行阶段放到后续，不在 R14-3 做；
- 真实 `seen_fingerprints` compact 执行阶段放到后续，不在 R14-5 做；
