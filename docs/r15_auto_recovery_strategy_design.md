...
| `cache_write_failed` | `safe_auto_recover` | 仅在项目显式允许 `fix-cache-1`、确认是可选缓存写入失败且可降级为内存缓存时考虑；通用 `disk_full` 仍人工升级。 |
| `optional_dependency_missing` | `safe_auto_recover` | 仅在项目显式允许 `fix-optional-dep-1`、确认存在 fallback 或可选集成开关时考虑；通用 `python_env` 仍人工升级。 |
| `worker_overload` | `safe_auto_recover` | 仅在项目显式允许 `fix-worker-1`、确认只是配置化 worker 并发过高时考虑；主机级资源不足仍人工升级。 |
