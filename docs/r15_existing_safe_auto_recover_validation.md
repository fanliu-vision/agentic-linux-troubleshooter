...
`fix-gpu-1` 已从 registry domain policy、project policy overlay、apply executor 和 guarded dry-run allowlist 确认。旧 `RemediationPolicy.DEFAULT_FIX_MAPPING` 仅作为 registry 派生的兼容别名保留一个 release 周期。未新增任何临时 fix id。
