# R16-0 safe recovery registry 收敛说明
...
- policy safe candidate event 集合；
- legacy `RemediationPolicy.DEFAULT_FIX_MAPPING` 中的 safe 映射；
...
执行层安全边界不变：真实恢复仍只允许项目内 JSON 字段编辑，并且必须经过 R15 runtime gate、precheck、cooldown、rollback 和 audit。
...
R16-1 新增企业 safe 域时，应先向 registry 增加 `SafeRecoverySpec`，再补 detector fixture、policy/gate 测试、apply/rollback 测试和 live dry-run 证据。
