...
R16-0 已完成 safe recovery registry 收敛。R16 后续新增恢复域时，必须先通过本文准入，再进入 registry、测试和 live dry-run 验证。
...
- 必须经过 R15 runtime gate、precheck、cooldown、audit；
...
| allowlist | fix_id 必须在 registry、runtime policy、项目 allowlist 中同时存在 | 阻断 |
